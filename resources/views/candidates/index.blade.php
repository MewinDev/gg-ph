    ...
    <div class="overflow-hidden lg:rounded-lg pb-10 pt-3 px-2">
        <div class="p-6 text-gray-900 dark:text-gray-100">
            ...
            <header class="mx-auto max-w-screen-xl text-left pb-5">
                <h2 class="mb-4 text-2xl md:text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white capitalize">{{ $electionTypeName }} Candidates</h2>
            ...
            <section class="mx-auto max-w-screen-xl">
                <div class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10">
                    @foreach ($candidates as $key => $candidate)
                        <x-templates.card :candidate="$candidate" href="{{ route('candidates.show', [urlencode($electionTypeName), str_replace('+', '-', urlencode($candidate->position->pos_name)), urlencode($candidate->id)]) }}"/>
                    @endforeach
                </div>
            </section>
        </div>
