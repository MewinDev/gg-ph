<div class="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 p-5 rounded-md">
    ...
    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Assign position to the candidate.</p>
        ...
        @foreach ($grouped as $typeName => $positions)
            ...
            @foreach ($positions as $position)
                <div class="flex items-center">
                    <input type="radio" id="{{ $position->id }}" name="position" value="{{ $position->id }}" class="hidden peer" required />
